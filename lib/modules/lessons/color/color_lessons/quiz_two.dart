...
import 'package:sign_buddy/modules/lessons/color/color_lessons/quiz_three.dart';
...
class ColorQuizTwo extends StatefulWidget {
  final String lessonName;

  const ColorQuizTwo({Key? key, required this.lessonName}) : super(key: key);

  @override
  State<ColorQuizTwo> createState() => _ColorQuizTwoState();
}

class _ColorQuizTwoState extends State<ColorQuizTwo> {
  ...
  Future<void> getProgress(String lessonName) async {
    try {
      ...
      print('Error reading colors_lesson.json: $e');
      if(mounted) {
      setState(() {
        isLoading = false;
      });
 ...
 void getContent3DataByName(String lessonName) async {
  ...
  void _nextPage() async {
    Navigator.pushReplacement(
      context,
      SlidePageRoute(page: ColorQuizThree(lessonName: widget.lessonName)),
    );
