...
class _ColorLessonState extends State<ColorLesson> {
  List colorLessonName = [];
  List colorLessonProgress = [];
 ...
 @override
  ...
  Future<void> colorLessons() async {
    ...
    try {
      ...
      for (QueryDocumentSnapshot<Map<String, dynamic>> doc
        ...
        colorNames.add(lessonData['name'] as String);
        unlockedLessons.add(lessonData['isUnlocked'] as bool);
        colorProgress.add(lessonData['progress'] as int);
      }
      if (mounted) {
        setState(() {
          colorLessonName = colorNames;
          unlockedColorLessons = unlockedLessons;
          colorLessonProgress = colorProgress;
        });
      }
        ...
        print('Error updating local color lessons: $e');
    }
  }
                  ...
                  child: Row(
                    ...
                    children: [
                      Container(
                       ...
                       Visibility(
                        child: Text(
                          isEnglish ? 'Learn Color' : 'Matuto ng Kulay',
                          style: TextStyle(
                            color: Colors.white,
                            fontSize: 20,
                            fontWeight: FontWeight.bold,
            ...
            delegate: SliverChildBuilderDelegate(
              (context, index) {
                if (isLoading) {
                  ...
                  final lessonName = colorLessonName[index];
                  final isUnlocked = unlockedColorLessons[index];
                  final progress = colorLessonProgress[index] / 100; // Normalize progress to a value between 0 and 1
                  return Padding(
              ...
              childCount: isLoading ? 1 :colorLessonName.length, // Show 1 item if loading, otherwise show the lesson names
            ),
          ),
        ],
      ),
