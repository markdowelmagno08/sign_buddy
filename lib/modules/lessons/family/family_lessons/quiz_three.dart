...
import 'package:sign_buddy/modules/lessons/family/family_lessons/quiz_four.dart';
...
class FamilyQuizThree extends StatefulWidget {
  final String lessonName;

  const FamilyQuizThree({Key? key, required this.lessonName}) : super(key: key);

  @override
  State<FamilyQuizThree> createState() => _FamilyQuizFourState();
}

class _FamilyQuizFourState extends State<FamilyQuizThree> {
  ...
  Future<void> getProgress(String lessonName) async {
    try {
      ...
      await FamilyLessonFireStore(userId: userId!)
      ...
      if (lessonData != null) {
        ...
        print(
            'By Progress: Family lesson "$lessonName" was not found within the Firestore.');
        isLoading = true;
      }
    } catch (e) {
  ...
  void _checkAnswer() async {
    ...
    if (isAnswerCorrect) {
      if (progress < 79) {
        ...
        FamilyLessonFireStore(userId: uid).incrementProgressValue(widget.lessonName, isEnglish ? "en" : "ph", 20);
    ...
    showResultSnackbar(context, resultMessage, icon, () {
      if (isAnswerCorrect) {
        ...
        Navigator.pushReplacement(
          context,
          SlidePageRoute(page: FamilyLessonOne(lessonName: widget.lessonName)),
        );
      }
    });
  }
  ...
  void _nextPage() {
    ...
    Navigator.pushReplacement(
    context, SlidePageRoute(page: FamilyQuizFour(lessonName: widget.lessonName))
    );
    if(mounted) {
      setState(() {
        selectedOption = '';
      ...
      appBar: AppBar(
          ...
          leading: IconButton(
            ...
            onPressed: () {
              ...
              Navigator.pushReplacement(context, SlidePageRoute(page: Family()));
            },
          ),
        ),
      body: Padding(
