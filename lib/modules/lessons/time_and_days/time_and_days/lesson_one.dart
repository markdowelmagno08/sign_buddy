...
import 'package:sign_buddy/modules/firestore_data/lesson_time_and_days.dart';
import 'package:sign_buddy/modules/lessons/time_and_days/time-and-days.dart';
import 'package:sign_buddy/modules/sharedwidget/page_transition.dart';
import 'package:sign_buddy/modules/widgets/back_button.dart';
import 'package:cached_video_player/cached_video_player.dart';

class TimeAndDaysLessonOne extends StatefulWidget {
  final String lessonName;

  const TimeAndDaysLessonOne({Key? key, required this.lessonName}) : super(key: key);

  @override
  State<TimeAndDaysLessonOne> createState() => _TimeAndDaysLessonTwoState();
}

class _TimeAndDaysLessonTwoState extends State<TimeAndDaysLessonOne> {
  ...
  Future<void> getProgress(String lessonName) async {
    try {
      ...
      Map<String, dynamic>? lessonData =
          await TimeAndDaysLessonFireStore(userId: userId!)
      ...
      if (lessonData != null) {
        ...
        print(
            'By Progress: Time and Days lesson "$lessonName" was not found within the Firestore.');
        isLoading = true;
      }
    } catch (e) {
      print('Error reading time_and_days_lessons.json: $e');
      setState(() {
        isLoading = false;
      });
    }
    ...
    try {
      ...
      if (lessonData != null && lessonData.containsKey('content1')) {
        ...
        if (mounted) {
          ...
          print(
              'By Content: Time and date lesson "$lessonName" was not found within the Firestore.');
          isLoading = true;
        }
      }
    } catch (e) {
      print('Error reading time_and_date_lessons.json: $e');
      if (mounted) {
        setState(() {
          isLoading = true;
        });
        ...
        child: Column(
          ...
          children: [
            ...
            Align(
              ...
              child: CustomBackButton(
                onPressed: () {
                  Navigator.pushReplacement(
                    context,
                    SlidePageRoute(page: TimeAndDays()),
                  );
                },
              ),
            ),
            const SizedBox(height: 70),
              ...
              alignment: Alignment.topLeft,
              child: Text(
                'Time and Days lesson for: "${widget.lessonName}"', 
                style: TextStyle(
                  fontSize: 20, 
                  ...
                  onPressed: isLoading || contentVideo.isEmpty
                      ...
                      : () async {
                          if (progress >= 31) {
                            ...
                            TimeAndDaysLessonFireStore(userId: uid)
