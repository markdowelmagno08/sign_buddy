...
import 'package:sign_buddy/modules/lessons/time_and_days/time_and_days/lesson_one.dart';
...
class _TimeAndDaysState extends State<TimeAndDays> {
  List timeLessonName = [];
  List timeLessonProgress = [];
  List unlockedTimeLessons = [];
  bool isEnglish = true;
  bool isLoading = true;
  ...
  Future<void> timeLessons() async {
    ...
    try {
      ...
      for (QueryDocumentSnapshot<Map<String, dynamic>> doc
        ...
        timeNames.add(lessonData['name'] as String);
        unlockedLessons.add(lessonData['isUnlocked'] as bool);
        timeProgress.add(lessonData['progress'] as int);
      }
      if (mounted) {
        setState(() {
          timeLessonName = timeNames;
          unlockedTimeLessons = unlockedLessons;
          timeLessonProgress = timeProgress;
        });
      }
        ...
        print('Error updating local time and days lessons: $e');
    }
  }
                  ...
                  child: Row(
                    ...
                    children: [
                      Container(
                       ...
                       Visibility(
                        child: Text(
                          isEnglish ? 'Learn Time and Days' : 'Matuto ng Oras at Araw',
                          style: TextStyle(
                            color: Colors.white,
                            fontSize: 18,
                            fontWeight: FontWeight.bold,
            ...
            delegate: SliverChildBuilderDelegate(
              (context, index) {
                if (isLoading) {
                  ...
                  final lessonName = timeLessonName[index];
                  final isUnlocked = unlockedTimeLessons[index];
                  final progress = timeLessonProgress[index] / 100; // Normalize progress to a value between 0 and 1
                  return Padding(
              ...
              childCount: isLoading ? 1 :timeLessonName.length, // Show 1 item if loading, otherwise show the lesson names
            ),
          ),
        ],
      ),
